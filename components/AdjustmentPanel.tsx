...
const ModificationsPanel: React.FC<ModificationsPanelProps> = ({ onApplyModification, isLoading, presets }) => {
  return (
    <div className="grid grid-cols-3 sm:grid-cols-4 gap-2 p-1 animate-fade-in">
      {presets.map(preset => (
        <button
          key={preset.name}
          onClick={() => onApplyModification(preset.prompt)}
          disabled={isLoading}
          ...
          aria-label={`Apply ${preset.name} modification`}
        >
